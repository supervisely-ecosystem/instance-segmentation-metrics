...
confusion_matrix_tab = Container([confusion_matrix_widget, confusion_matrix_table], gap=20)
...
metrics_tabs = Tabs(
    ...
    [confusion_matrix_tab, overall_tab, per_class_tab],
)
metrics_card = Card(
...
def on_confusion_matrix_click(cell: ConfusionMatrix.ClickedDataPoint):
    ...
    res = df[(df["gt_class"] == gt_class) & (df["pred_class"] == pred_class)]
    res["dataset"] = dataset_names_gt[res["dataset_id"]].values
    res = res.drop(columns=["gt_label_id", "pred_label_id", "dataset_id"])

    confusion_matrix_table._update_table_data(input_data=res)
    confusion_matrix_table.update_data()
    DataJson().send_changes()
